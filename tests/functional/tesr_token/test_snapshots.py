...
def test_snapshots_when_mint(tesr, gov, minter):
  ...
  assert tesr.total_supply_at.call(0) == token_supply - amount_to_burn + amount_to_mint

  tesr.mint(gov, amount_to_mint, {"from": minter})
  tesr.snapshot()

  assert tesr.total_supply_at.call(1) == token_supply - amount_to_burn + (2 * amount_to_mint)
...
def test_snapshots_when_transfer(tesr, gov, rando):
  ...
  assert tesr.balance_of_at.call(gov, 1) == token_supply - amount_to_transfer + (amount_to_transfer / 2)
  assert tesr.balance_of_at.call(rando, 1) == amount_to_transfer - (amount_to_transfer / 2)
