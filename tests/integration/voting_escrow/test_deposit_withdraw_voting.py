...
class StateMachine:
    ...
    st_value = strategy("uint64", min_value=2)
    ...
    def rule_withdraw(self, st_account):
        ...
        """
        balance_before_withdrawal = self.token.balanceOf(st_account)
    ...
    def invariant_escrow_current_balances(self):
        ...
        for acct in self.accounts:
            ...
            if data["unlock_time"] > timestamp and data["value_locked"] // MAX_TIME > 0:
                assert balance
            elif not data["value_locked"] or data["unlock_time"] <= timestamp:
                assert not balance
