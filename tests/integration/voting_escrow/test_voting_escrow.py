...
def test_voting_powers(web3, chain, accounts, tesr_token, voting_escrow):
    """
    ...
    amount = 1000 * 10 ** 18
    tesr_token.transfer(bob, amount, {"from": alice})
    stages = {}

    tesr_token.approve(voting_escrow.address, amount * 10, {"from": alice})
    tesr_token.approve(voting_escrow.address, amount * 10, {"from": bob})
